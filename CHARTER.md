# Project Charter — Air-Gapped Codex + vLLM
...
- Runs a **local LLM** (via vLLM) for inference—**including CPU-only** so it works on machines without GPUs.
...
- Can be **prepared once** on a connected machine (bootstrap: download model, install deps, generate config) and then **copied as a whole** (e.g. via approved media) into the air-gapped workspace. No network is required on the target side.
...
2. **CPU inference is a first-class scenario**  
   Many secure workstations have no GPU. The project must support **CPU-only** inference so that operators without GPUs can still use the agent. GPU remains recommended for speed, but the product is usable on CPU.

3. **Codex as the agent interface**  
   We use **Codex CLI** as the agent front-end (tools, exec, planning, sandboxing). Our job is to make Codex work against a **local** model served by vLLM (Responses API), not to replace Codex with a different agent stack.
...
5. **Transparency and auditability**  
   No telemetry or phone-home. All processing is local. The stack (vLLM, Codex, model choice) is documented and version-pinned where it matters (see VERSIONS.md) so operators and security reviewers can reason about what runs in the secure zone.
...
- **Local serving:** vLLM serving a chosen model (default and optional presets), with **GPU and CPU** support.  
- **Codex integration:** Config and scripts so Codex uses the local vLLM endpoint (Responses API), with state confined to the project (e.g. `CODEX_HOME`).  
...
- **Guarantee support for every model or hardware.** We document a baseline (vLLM version, Codex version, optional model presets) and keep that path tested; exotic setups may require user adaptation.
...
- An operator in an air-gapped environment can:  
  ...
  - Start the local server (GPU or CPU).  
