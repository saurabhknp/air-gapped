...
会下载 llama.cpp 预编译二进制（Linux x64 CPU）、下载默认 GGUF 模型（[Edge-Quant/Nanbeige4.1-3B-Q4_K_M-GGUF](https://huggingface.co/Edge-Quant/Nanbeige4.1-3B-Q4_K_M-GGUF)，约 2GB）并生成配置。看到 `[bootstrap] Done. Next: ...` 即完成。
...
等服务就绪（例如在浏览器打开 http://127.0.0.1:28080）。首次启动会加载模型，稍等即可。
...
**默认模型：** [Edge-Quant/Nanbeige4.1-3B-Q4_K_M-GGUF](https://huggingface.co/Edge-Quant/Nanbeige4.1-3B-Q4_K_M-GGUF)。使用其他 GGUF 仓库：

```bash
HF_MODEL_REPO_ID=owner/repo-name ./bootstrap.sh
```

**端口：** 默认 28080。可设置 `LLAMA_PORT=28081 ./start-llama-server.sh` 与 `LLAMA_PORT=28081 ./run-codex.sh exec "..."`。

**上下文长度/线程：** 启动服务时可设置 `LLAMA_CTX_SIZE=8192`、`LLAMA_THREADS=8`（见 `start-llama-server.sh`）。
...
| llama-server 构建失败 | 确保已安装 `cmake` 和 C++ 编译器。Linux 示例：`sudo apt install build-essential cmake`。 |
