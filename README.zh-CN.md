...
会创建 Python 环境、下载默认模型（GGUF Q4_K_M 量化，约 2GB）并生成配置。看到 `[bootstrap] Done. Next: ...` 即完成。
...
**默认：** CPU 模式 + GGUF 量化模型；`./start-vllm.sh` 会自动探测本机 CPU 核数与内存并设置线程与 KV 缓存。

**使用全精度模型（如搭配 GPU）：**

```bash
./bootstrap.sh 1          # 或：./bootstrap.sh nanbeige
...
需要覆盖自动探测的 CPU 资源时，可设置：`VLLM_CPU_NUM_THREADS=8`、`VLLM_CPU_KVCACHE_SPACE=4`（单位 GiB）。详见 [docs/README.md](docs/README.md#controlling-cpu-inference-threads-and-memory)。
...
| 内存不足 (OOM) | GPU：可减小 `VLLM_MAX_MODEL_LEN` 或降低 `VLLM_GPU_MEMORY_UTILIZATION`。CPU：设置 `VLLM_CPU_KVCACHE_SPACE`（单位 GiB）和/或 `VLLM_MAX_MODEL_LEN=4096`。 |
...
- **[CHANGELOG.md](CHANGELOG.md)** — 发布历史。当前版本：**0.2.0**（见 [VERSION](VERSION)）。
...
| `CHANGELOG.md`、`VERSION` | 发布历史与当前版本（0.2.0）。 |
