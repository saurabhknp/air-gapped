...
- 使用 `USE_VLLM=1` 时的默认 vLLM 模型：[Edge-Quant/Nanbeige4.1-3B-Q4_K_M-GGUF](https://huggingface.co/Edge-Quant/Nanbeige4.1-3B-Q4_K_M-GGUF)（与 CPU 相同）。启动时默认最多 1 并发、自动检测上下文长度、高显存占用 (0.95)。tokenizer 从基础模型自动下载。
...
**说明：** CPU 与 GPU 默认使用相同的 GGUF 模型。如需为 GPU 使用不同的 HF 模型，可通过 `VLLM_MODEL` 覆盖。
